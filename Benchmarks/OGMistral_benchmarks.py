...
def load_model():
    ...
    try:
        ...
        return tokenizer, model, device, load_time
...
def main():
    print("Loading Mistral-7B-Instruct-v0.1 model from local directory...")
    tokenizer, model, device, load_time = load_model()
    disk_size = get_folder_size("./mistral")
    print(f"Model loading time: {load_time:.2f} seconds")
    print(f"Disk space usage: {disk_size:.2f} GB")
    print("Model loaded. Type 'exit' to quit.")

    with open("metrics_mistral.txt", "w") as f:
        f.write(f"Model: Mistral-7B-Instruct-v0.1\n")
    ...
    while True:
        ...
        try:
            ...
            response, response_time, throughput, memory_usage, word_count = generate_response(model, tokenizer, device, prompt)
            print(f"Chatbot: {response}")
            print(f"Response time: {response_time:.2f} seconds")
            print(f"Throughput: {throughput:.2f} tokens/second")
            print(f"Memory usage: {memory_usage:.2f} MB")
            print(f"Response word count: {word_count}")
            with open("metrics_mistral.txt", "a") as f:
